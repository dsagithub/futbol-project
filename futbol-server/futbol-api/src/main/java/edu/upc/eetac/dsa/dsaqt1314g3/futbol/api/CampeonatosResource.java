...
public class CampeonatosResource {
	...
	public EquipoCollection getEquipos(
			@PathParam("idcampeonato") String idcamp,
		...
		if (ioffset != 0) {
			...
			equipos.addLink(EquiposLinkBuilder.buildURIEquipoCampeonato(
					uriInfo, prevoffset, length, pattern, "prev", idcamp));
		}
		equipos.addLink(EquiposLinkBuilder.buildURIEquipoCampeonato(uriInfo,
				offset, length, pattern, "self", idcamp));
		String nextoffset = "" + (ioffset + ilength);
		if (ilength <= icount) {
			equipos.addLink(EquiposLinkBuilder.buildURIEquipoCampeonato(
					uriInfo, nextoffset, length, pattern, "next", idcamp));
		}
		return equipos;
	}
	...
	public Campeonatos getCampeonatos(
		...
		try {
			...
			if (rs.next()) {
				...
				campeonato.addLink(CampeonatosLinkBuilder.buildURICampeonatoId(
						uriInfo, "self", campeonato.getIdcampeonatos()));
				campeonato.addLink(CampeonatosLinkBuilder.buildURICalendarioId(
						uriInfo, "Calendario", campeonato.getIdcampeonatos(),
						offset, length, pattern));
			} else {
	...
	@DELETE
	@Path("/{idcampeonato}")
	public void borrarcampeonato(@PathParam("idcampeonato") int idcampeonato) {
		Connection conn = null;
		...
		try {
			...
			if (rs2 == 0)
				throw new ClubNotFoundException(
						"No existe ningún campeonato con esa id");
	...
	public Campeonatos crearCampeonato(Campeonatos campeonato) {
		...
		if (campeonato.getNombre().length() > 45) {
			throw new BadRequestException(
					"El nombre del campeonato ha de ser menor de 45 carácteres");
		}

		if (campeonato.getNombre().length() == 0) {
			throw new BadRequestException(
					"El nombre del campeonato no puede estar vacío");
		}
	...
	@PathParam("idcampeonato") int idcampeonato, Campeonatos campeonato) {
		...
		try {
			...
			String sql = "update campeonatos set campeonatos.nombre='"
					+ campeonato.getNombre()
			...
			campeonato.addLink(CampeonatosLinkBuilder.buildURICampeonatoId(
					uriInfo, "self", campeonato.getIdcampeonatos()));
			campeonato.addLink(CampeonatosLinkBuilder.buildURICalendarioId(
					uriInfo, "Calendario", campeonato.getIdcampeonatos(), "0",
					"5", null));
			stmt.close();
			conn.close();
		} catch (SQLException e) {
	...
	public CampeonatosCollection getcampeonatos(
		...
		try {
			...
			if (pattern != null) {
				sql = "select * from campeonatos where nombre like '%" + pattern
						+ "%' LIMIT " + offset + "," + length;
			} else {
				sql = "select * from campeonatos LIMIT " + offset + ","
						+ length;
			}
			...
			while (rs.next()) {
				...
				if (ilength <= icount) {
					...
					ccol.addLink(CampeonatosLinkBuilder.buildURICampeonatos(
							uriInfo, nextoffset, length, pattern, "next"));
			...
			if (ioffset == 0) {
				ccol.addLink(CampeonatosLinkBuilder.buildURICampeonatos(
						uriInfo, offset, length, pattern, "self"));
			} else {

				if (cuenta >= 0) {
					ccol.addLink(CampeonatosLinkBuilder.buildURICampeonatos(
							uriInfo, prevoffset, length, pattern, "prev"));
				} else {

					ccol.addLink(CampeonatosLinkBuilder.buildURICampeonatos(
							uriInfo, "0", length, pattern, "prev"));
